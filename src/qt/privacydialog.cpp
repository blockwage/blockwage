...
#include "zblockwagecontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zBWG ought to be enough for anybody." - Bill Gates, 2017
    ui->zBWGpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzBWGSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzBWG->setEnabled(false);
        ui->pushButtonMintzBWG->setToolTip(tr("zBWG is currently disabled due to maintenance."));

        ui->pushButtonSpendzBWG->setEnabled(false);
        ui->pushButtonSpendzBWG->setToolTip(tr("zBWG is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zBWGpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzBWG_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zBWG is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zBWG...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zBWG in ") +
...
void PrivacyDialog::on_pushButtonSpendzBWG_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zBWG is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zBWG
        sendzBWG();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zBWG
    sendzBWG();
}

void PrivacyDialog::on_pushButtonZBlockwageControl_clicked()
{
    ZBlockwageControlDialog* zBWGControl = new ZBlockwageControlDialog(this);
    zBWGControl->setModel(walletModel);
    zBWGControl->exec();
}

void PrivacyDialog::setZBlockwageControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzBWGSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzBWG()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Blockwage Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zBWGpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zBWG is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " BLOCKWAGE </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zBWG</b>";
    ...
    // use mints from zBWG selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZBlockwageControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZBlockwageControlDialog::GetSelectedMints();
    }
    ...
    // Clear zblockwage selector in case it was used
    ZBlockwageControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zBWG Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Blockwage, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zBWG Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zBWG </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zBWG "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zBWG "));
    ui->labelzBWGAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzBWGSyncStatus->setVisible(fShow);
}
