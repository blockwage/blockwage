...
#include "ui_zblockwagecontroldialog.h"
...
std::list<std::string> ZBlockwageControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZBlockwageControlDialog::listMints;

ZBlockwageControlDialog::ZBlockwageControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZBlockwageControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZBlockwageControlDialog::~ZBlockwageControlDialog()
{
    delete ui;
}

void ZBlockwageControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZBlockwageControlDialog::updateList()
{
...
void ZBlockwageControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZBlockwageControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZBlockwage_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZBlockwageControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZBlockwageControlDialog::GetSelectedMints()
{
...
void ZBlockwageControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
